...
package com.hifong.war.commands.building.market
{
	...
	public final class TransportResouceByCityNameCommand extends SequenceCommand implements ICommand, IResponder
	{
		...
		{
			var evt:TransportResouceByCityNameEvent = event as TransportResouceByCityNameEvent;
			var delegate:CityDelegate = new CityDelegate(this);
			
			this.resTransportation = evt.resTransportation;
			this.cityID = evt.cityID;
			this.targetCityName = evt.targetCityName;
			
			delegate.getCityIDByCityName(evt.targetCityName);
		}
		
		public function result(data:Object) : void
		{
			if(data.result==null){
				MsgBox.showMessage("目标城市 " + targetCityName + " 不存在");
			}else{
